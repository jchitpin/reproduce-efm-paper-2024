...
metabolic fluxes in terms of the steady state flow of their molecular
...
liver cancer cell line. Our results systematically characterize source
metabolite remodelling and reveal the most active pathways of HepG2
glutamine metabolism. We anticipate our method will be useful for
quantifying cellular metabolism as high-throughput fluxomic analyses
...
We outline our computational pipeline to enumerate and identify AEFM
weights in Figure 1c. In step 1, we require as inputs the metabolic
...
strings, and (optionally) the steady state fluxes. From the stoichiometry
matrix and SMILES strings, we construct reaction SMILES strings which are
imputed into the atom mapping program
...
set the transition probabilities equal to the atomic flux through that
reaction. For example, 10\SI{}{\milli\molar} of input glucose flux would
correspond with 10\SI{}{\milli\molar} of atomic flux through each glucose
carbon. From each atomic transition graph, we construct the corresponding
ACHMC to enumerate all AEFMs involving that source metabolite atom in step
3. Finally, the AEFM weights are computed in step 4 by steady state
   analysis of the ACHMC. If no steady state fluxes are imputed, only the
   enumerated AEFMs in step 3 are returned.
...
of network size. These networks included a metabolic model of \textit{E.
...
magnitude more carbon AEFMs which suggested a greater degree of network
connectivity. Collapsing across carbon AEFMs and nitrogen AEFMs, we
observed a consistent ratio between the number of ACHMC transitions and
ACHMC states ($\mu_C = 1.43, \sigma^2_C = 0.14$; $\mu_N = 1.53, \sigma^2_N
= 0.16$).
...
remodelling. Our approach was furthermore significantly faster than the
state-of-the-art program FluxModeCalculator. Our benchmarking showed ACHMC
computation times to be $10^3-10^4$ times faster than FluxModeCalculator
with both programs running in serial. Running FluxModeCalculator in
parallel with an optimal number of threads did improve run times by 8.2
8.6 times (Figure EV2), yet remained over $10^1-10^2$ slower than
  enumerating the corresponding network carbon and nitrogen AEFMs.
...
Having systematically enumerated all carbon and nitrogen EFMs across the five
GEMs, we next turned to characterizing their structural properties. Molecular
EFMs can be divided into pathway that span source and sink metabolites or
internal loops within the metabolic network such as pairs of reversible
reactions. AEFMs can be further classified into those that transit sets of
distinct metabolites or those that revisit the same metabolite, albeit in
different atomic positions. An example of a common source-to-sink atomic EFM
with metabolite revisitations is a carbon AEFM tracing glucose remodelling
through two turns of the TCA cycle; on the second turn, the position of the
carbon atom shifts within the TCA intermediates, resulting in different
isotopomers and therefore a metabolite revisitation under our definition.

To obtain a broad understanding of the types of AEFMs within the five GEMs, we
categorized them into source-to-sink pathways and internal cycles with or
without metabolite revisitations in different atomic positions. Figure
3a-b shows the counts for each AEFM classes within each GEM. We observed
that the majority of carbon AEFMs were source-to-sink pathways with or
without metabolite revisitations. The number of these pathways without
metabolite revisitations tended to scale as a function of metabolic
network size. The number of looped carbon AEFMs without metabolite
revisitations increased slightly while those with revisitations varied
across the five GEMs. Similar findings were observed for the nitrogen
AEFMs, although looped pathways with metabolite revisitations were only
observed in the iIT341 and iSB619 networks. The iAB RBC 283 dataset was
unique in that it only contained source-to-sink and looped pathways with
no metabolite revisitations. This is likely due to the lack of cellular
organelles in red blood cells and therefore no cyclic reactions associated
with metabolite transport between compartments or mitochondrial TCA
activity.
...
short with few pathways exceeding 50 reactions. In contrast, the HepG2
...
and nitrogen ratios over $10^2$. These ratios tended be closer to 1 for
source metabolites containing fewer carbons, such as glycine which is
equivalently metabolized across the same AEFMs across all networks.
...
identifying alternative pathways of glutamine metabolism as many cancer cells,
including HepG2 cells, uptake more glutamine than is required for biomass
production~\citep{YeYanyan2023Gmri,YooHeeChan2020Gric}. Using our method, we
decomposed the set of steady state fluxes onto the 241,085 glutamine carbon
AEFMs characterizing the HepG2 metabolic network and sought to identify the
most dominant pathways carrying the greatest glutamine carbon flow through
the network. Figure 4a shows the cumulative explained mass flow of each
...
of each glutamine carbon at 99\% because we observed that hundreds to
thousands of carbon AEFMs collectively explained the remaining 1\%
glutamine mass flow. The resulting curves indicate that the majority of
glutamine carbon remodelling is explained by a surprisingly small number
of glutamine AEFMs, especially since our Markovian flux decomposition
assigns some weight to all network pathways. The top 5 ranked AEFMs across
each glutamine carbon (25 pathways altogether) explained over half
(50.2\%) of the total glutamine carbon mass flow with the subsequent
...
derived from other amino acids as well as glucose (Figure EV3). For some
...
\citeauthor{ArnoldPaigeK2022Anta}, who validated this pathway in
a different (non-small cell lung) cancer cell line by stable isotope
tracing experiments. This non-canonical TCA activity was also observed in
AEFM 2 in Figure 4e among the glutamine carbons 3 and 4. We also found
AEFMs 1, 3, and 5 in both Figure 4d and Figure 4e also corresponded to the
malate-asparate shuttle. Only the terminal glutamine carbon in Figure 4e
...
Having mapped each AEFM to an existing metabolic subsystem, we asked whether we
could combine the results across all Sankey diagrams into a simplified
metabolic model describing bulk glutamine remodelling in the HepG2 flux
...
AEFMs that overlapped with reactions within these subsystems. 

Our results suggest input glutamine is converted into cytosolic glutamate which
is transported into the mitochondria through aspartate/glutamate antiporters
encoded by \textit{SLC25A12} and \textit{SLC25A13}. Mitochondrial glutamate is
then converted into TCA intermediate alpha-ketoglutarate and citrate. Instead
of following the canonical reactions of the TCA cycle, the mitochondrial
citrate is exported to the cytosol where it can participate in non-canonical
TCA cycle activity or the malate-aspartate shuttle to re-enter the
mitochondria. Taken together, the dominant carbon AEFMs predicted by our model
suggest HepG2 glutamine is conserved within TCA intermediates that are
regenerated by shuttling mitochondrial citrate to the cytosol.
...
constraints may change across fluxes estimated from different biological
conditions. Our results suggest that enumerating the shortest pathways
would fail to recover the majority of TCA pathways, as the top five ranked
AEFMs across all glutamine carbons averaged 6 reactions in length. By
...
within minutes or hours on desktop computing resources. We are hopeful
...
\subsubsection*{Extracting SMILES strings}

SMILES structures, unfortunately, were not encoded in the BiGG SBML files
nor the HepG2 dataset. We manually extracted SMILES strings
corresponding to each metabolite across all GEMs. Isomeric SMILES strings
were taken from PubChem~\citep{KimSunghwan2023P2u}, the Human Metabolome
...
modification would not alter atom mappings, especially since
RXNMapper does not map hydrogen atoms unless they are explicitly
represented in the SMILES strings. SMILES strings were also not assigned
to metabolites with ambiguous structures nor pseudometabolites with no
known chemical structure. All pseudometabolites and reactions involving
pseudometabolites were removed from the stoichiometry matrix (Table 2).
...
substrates and products is denoted by the symbol `>>'. The substrate
and product SMILES strings are concatenated in order of metabolite
appearance within the stoichiometry matrix, and multiple stoichiometric
copies are consecutively repeated in the reaction SMILES strings.
Non-integer stoichiometric coefficients are therefore not allowed and these
pseudoreactions were removed from the stoichiometry matrix (Table 2). We
manually checked several reactions to ensure the assigned atom mappings were
correct. Crucially, we found RXNMapper to fail on nearly all classes of
transaminase reactions that were not ATP-dependent. This problem has previously
been reported in an atom mapping benchmark study prior to the publication of
RXNMapper~\citep{PreciatGonzalezGermanA.2017Ceoa}. These transaminase reactions
were identified in all metabolic networks and manually atom mapped based
on the KEGG reaction mappings (Figure EV6).
...
model, we impose additional requirements on the input metabolites and
reactions. The 3D structure of each metabolite must be known and encoded
as a SMILES structure. We recommend, where applicable, using isomeric
SMILES structures to avoid ambiguous or incorrect atom mappings. We
further require integer-valued stoichiometric coefficients because we
consider atoms to be indivisible entities from which we enumerate their
AEFMs. We lastly require the input metabolic flux network to be open such
that external fluxes enter and leave via source and sink metabolites. We
note this is generally not a problem in large-scale metabolic networks;
our requirement is not a technical limitation of our method but enforced
to promote the enumeration of all source-to-sink AEFMs remodelling
nutrient sources.
...
\noindent Fig 2: AEFMs are computationally tractable compared to
molecular EFMs in five GEMs. (a) Number of atomic and molecular EFMs in
the five GEMs. (b) The run time of our MarkovWeightedEFMs.jl method versus
FluxModeCalculator running in serial. (c) Serial run time scaling of
MarkovWeightedEFMs.jl as a function of the number of atomic EFMs. (D)
Speedup of selected ACHMCs as a functional of additional threads.
