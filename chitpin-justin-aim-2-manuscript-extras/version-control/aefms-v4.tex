...
non-small cell lung cancer (NSCLC), mouse embryonic, and C2C12 mouse
...
uniquely assign their weights when the steady state fluxes of the network
are known. In step 1, we require as inputs the metabolic network defined
by a set of metabolites and reactions encoded as a stoichiometry matrix,
the metabolite structures denoted as SMILES strings, and the steady state
...
stoichiometries (see Methods and Figure EV 5).
...
coli} (E. coli core; \cite{OrthJeffreyD2010RaUoDATASET}), human red blood
cells (iAB RBC 283; \cite{BordbarAarash2011iApdDATASET}), \textit{H.
pylori} (iIT341; \cite{ThieleInes2005EMRoDATASET}), \textit{S. aureus}
(iSB619; \cite{BeckerScottA2005GrotDATASET}), and human liver cancer cells
(HepG2; \cite{NilssonAvlant2020QaoaDATASET}). These models were
pre-processed to ensure unambiguous atom mappings across all reactions.
Namely, all metabolites with no known structures (pseudometabolites), and
...
connectivity between internal metabolites in the HepG2 network (Figure
EV1).
...
revisit the same metabolite, albeit in different atomic positions. This
notion of a ``metabolite revisitation'' is equivalent to an isotopomer in
stable isotope tracing~\citep{AntoniewiczMaciekR2018Agt1}. An example
source-to-sink AEFM with this property is a glucose-derived carbon AEFM
remodelling through two turns of the TCA cycle; on the second turn, the
position of the carbon atom shifts within the TCA intermediates, resulting
in different isotopomers~\citep{DuanLikun20221tas} and therefore
...
intermediates. These two observations may explain why the iAB RBC 283
...
a small peak spanning 2-12 reactions. We considered whether these two
peaks observed in all five GEMs were associated with source-to-sink and
looped AEFM classes. We tested this hypothesis by reclassifying the carbon
AEFMs spanning less than 5-12 reactions depending on the GEM (shorter
pathways), and the remaining carbon AEFMs (longer pathways). We found that
long pathways were associated with source-to-sink pathways, with shorter
pathways containing the majority of looped pathways. In fact, 65-95\% of
all longer pathways in each GEM were source-to-sink pathways with
metabolite revisitations, with this statistic increasing to 95-100\% when
including source-to-sink pathways without metabolite revisitations. Among
shorter pathways, we observed that roughly half or more of them were
looped carbon AEFMs with or without metabolite revisitations. Analysis of
the nitrogen AEFMs revealed only a single distribution of relatively short
pathways, since there are much fewer nitrogen-containing metabolites
across all GEMs.

We finally investigated how atoms within the same source metabolite may
...
class. We first computed the number of AEFMs for each carbon atom (resp.
number of paths. So for example, a ratio of 10 means that one of the
carbon atoms in the metabolite can transit the network in 10 times as many
different ways as the carbon with the fewest paths through the network.
Our results are shown in Figure 3e-f, where each dot corresponds to the
ratio for a given source metabolite. We found that a surprising number of
carbons and nitrogens can transit different pathways within their
respective source metabolites. Focusing on the source metabolites with
carbon and nitrogen ratios over 100, we observed many of these
corresponded to hydrophobic and positively-charged amino acids such as
phenlylalanine, methionine, arginine, and histidine. Source metabolites
containing fewer carbons or nitrogens tended to exhibit ratios closer to
1 with AEFMs involving the same metabolite subsets. Metabolites such as
  ...
  carbon AEFMs within each GEM. For each GEM, and not counting source
  metabolites with ratios of one, we computed median carbon ratios between
...
We finally highlight the potential of our AEFM method for understanding
nutrient source metabolism. Specifically, we analyzed the single set of
steady state fluxes from the HepG2 network to identify alternative
pathways of glutamine metabolism. This question is highly relevant since
many cancer cells, including HepG2, are dependend on this non-essential
...
centralized to a relatively small subset of AEFMs (Figure EV3). For some
source metabolites, we observed overlapping curves between carbons (e.g.
arginine, glycine, serine in Figure EV3), suggesting that individual
carbons were remodelled through the same AEFMs. Corroborating results from
Figure 3e-f, we observed many non-overlapping curves for the majority of
amino acids since their carbons could transit the network via different
...
can remodel into 202 species, based on the RXNMapper atom mapping
predictions, localized to the cytosol or mitochondria. We found that the
top 5 glutamine-derived carbon AEFMs consist of just 23 metabolites and
31 reactions (Figure 4b). These AEFMs generally consisted of cyclic
pathways involving TCA intermediates. Only two source-to-sink reactions
converting cytosolic glutamine towards the protein pool and carbon dioxide
through alpha-ketoglutarate decarboxylation in the mitochondria. 
...
cell lung cancer, mouse embryonic, and C2C12 mouse myoblast cells by
stable isotope tracing analysis~\citep{ArnoldPaigeK2022Anta}. This
non-canonical TCA activity was further observed in glutamine carbons
3 and 4 in AEFM 2 (Figure 4e). We also found AEFMs 1, 3, and 5 in Figure
  4d-e corresponded to the malate-asparate shuttle. Only the terminal
...
TCA cycle, non-canonical TCA cycle, and the malate-aspartate shuttle.
...
While statistical analyses, such as different AEFM weight analysis, may
reveal metabolic changes in individual pathways altered across conditions,
it is likely that changes in a metabolic pathway may impact weights in
multiple AEFMs. For example, the majority of AEFMs across distinct source
metabolites are not ``molecularly'' independent. By this, we imply that
some AEFMs may correspond to the same sequence of metabolites or different
...
product. We manually checked several reactions to ensure the assigned
atom mappings were correct. Crucially, we found RXNMapper to fail on
nearly all classes of transaminase reactions that were not ATP-dependent.
This problem has previously been reported in an atom mapping benchmark
study prior to the publication of
...
mapped based on the KEGG reaction mappings (Figure EV6).
...
state flux across the network. All pre-processing steps described above
...
to maintain atomic mass conservation (Figure EV 5). The resulting atomic
...
metabolite/atom position. In all cases, we set the atomic flux of the atom
transitioning from either stoichiometric copy equal to the reaction fluxes
to maintain atomic mass conservation (Figure EV 5).
...
number of AEFMs. (d) Speedup of selected ACHMCs as a functional of
additional threads.
