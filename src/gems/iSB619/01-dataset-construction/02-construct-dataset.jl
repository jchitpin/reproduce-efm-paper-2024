...
# (1) Load original iSB619 dataset. Stoichiometry matrix and
...
# NOTES:
# (A) Reaction index 141 (S Aureus Biomass based on B Subtilis precursors) is
# not atom balanced and is therefore removed from the model.
# ------------------------------------------------------------------------------
...
# Import file names
im_stoich = load * "stoich.csv"
im_mets   = load * "metabolites-compartments.csv"
im_rxns   = load * "reactions.csv"
#im_smiles = load * "smiles.csv"
im_smiles = load * "smiles-isomeric.csv"
...
# Stoichiometries are integers but so large they lead to overflow errors
to_rm = findall(==("S Aureus Biomass based on B Subtilis precursors"), rxns)
S = S[:, setdiff(1:size(S,2), to_rm)]
rxns = rxns[setdiff(1:length(rxns), to_rm)]
# ------------------------------------------------------------------------------
...
@info("Dropped rows pseudomets: $(length(i4.dropped_rows_pseudometabolites))") # 108
@info("Dropped cols pseudomets: $(length(i4.dropped_cols_pseudometabolites))") # 204
@info("Dropped cols rxnmapper:  $(length(i4.dropped_cols_rxnmapper_limit))")   # 13 + 1
