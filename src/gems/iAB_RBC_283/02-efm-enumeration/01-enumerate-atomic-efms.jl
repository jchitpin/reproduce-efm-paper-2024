...
# (1) Enumerates and saves the atomic EFMs in the iAB_RBC_283 network.
# ------------------------------------------------------------------------------
...
time_C_loc = "data/gems/$set/efm-enumeration-times-carbon.jld2"
time_N_loc = "data/gems/$set/efm-enumeration-times-nitrogen.jld2"
# ------------------------------------------------------------------------------
...
## ENUMERATE ATOMIC EFMS AND SAVE ----------------------------------------------
# Identify indices of all source metabolites and their carbon/nitrogen numbers
...
for k in eachindex(srcs)
    ...
    for l in 1:amax_C[srcs[k]]
        ...
        jldsave(saveC * "$k-" * mets[srcs[k]] * "/atomic-efms-$l.jld2"; res)
    end
    push!(ctimes, ctmes)
end
...
for k in eachindex(srcs)
    ...
    for l in 1:amax_N[srcs[k]]
        ...
        jldsave(saveN * "$k-" * mets[srcs[k]] * "/atomic-efms-$l.jld2"; res)
    end
    push!(ntimes, ntmes)
end
