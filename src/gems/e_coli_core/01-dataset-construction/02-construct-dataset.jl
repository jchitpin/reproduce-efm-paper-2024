...
# (1) Load original e_coli_core dataset. Stoichiometry matrix and
...
@info("Dropped rows pseudomets: $(length(i4.dropped_rows_pseudometabolites))") # 1
@info("Dropped cols pseudomets: $(length(i4.dropped_cols_pseudometabolites))") # 2
@info("Dropped cols rxnmapper:  $(length(i4.dropped_cols_rxnmapper_limit))")   # 0
