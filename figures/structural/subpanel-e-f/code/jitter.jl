...
# All GEMs
idx_carbon3 = Int64
idx_carbon3 = 0 # equals 16 source metabolites with only 1 carbon
for i in eachindex(C1)
    for j in 1:length(C1[i])
        if length(C1[i][j]) == 1
            global idx_carbon3 += 1
        end
    end
end
idx_carbon3
...
idx_nitrogen3 = Int64
idx_nitrogen2 = 0 # equals 108 source metabolites with only 1 nitrogen
for i in eachindex(N1)
    for j in 1:length(N1[i])
        if length(N1[i][j]) == 1
            global idx_nitrogen2 += 1
        end
    end
end
idx_nitrogen2
