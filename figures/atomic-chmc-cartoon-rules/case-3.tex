...
\begin{tikzpicture}[]
    ...
    % Product 2
    \node[ball,right=\multispace of b7,fill=cred] (b8) {$c_4$};

    % Substrate addition
    \draw[draw=none] (b7.east) -- node[] {$\bm{+}$} (b8.west);
    ...
    %% Atomic flux network
    \node[ball,right=2.27*\fluxspace of b8,fill=none] (b9)   {$a_1$};
    ...
    \node[ball,below=\fluxspace of b9,xshift=+0.4cm,fill=cred] (b11)  {$c_4$};
    ...
    \node[ball,right=2.60*\fluxspace of b9,fill=none]  (b12) {$a_2$};
    \node[ball,above=0cm of b12.east,rotate=90,circle split part fill={corange,cgrey},anchor=south] (b12)  {};
    \node[ball,right=2.60*\fluxspace of b9,fill=none]  (b12) {$a_2$};
    \node[ball,below=\fluxspace of b12,xshift=-0.4cm,fill=corange] (b13) {$b_2$};
    \node[ball,below=\fluxspace of b12,xshift=+0.4cm,fill=cgrey] (b14) {$b_3$};
    \draw[line width=1.0pt,-stealth'] (b12.south) -- node[above left,font=\Large] {$v$} (b13.north);
    \draw[line width=1.0pt,-stealth'] (b12.south) -- node[above right,font=\Large] {$v$} (b14.north);
\end{tikzpicture}
