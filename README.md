...
* Julia (minimum version 1.11.3).
* MATLAB (version 2018a or greater) is optional for reproducing the benchmarking studies.
...
* The HDF5-compressed data under data/gems/$<$gem-name$>$/atomic-efms-[carbon/nitrogen] is not exported due to file size constraints. However, these files will be regenerated by running the source scripts in the pipeline.
...
After some Julia updates, the following packages may need to be explicitly
...
*Atomic elementary flux modes explain the steady state flow of metabolites in flux networks*.
biorXiv preprint doi: 10.1101/2024.11.13.623484v1
